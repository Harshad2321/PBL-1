"""
Adaptive Learning System for Nurture Simulation
===============================================

Enables AI agents to learn from interactions and adapt their behavior.
- Learns user preferences and communication style
- Updates personality traits based on interactions
- Adapts response strategies over time
- Tracks patterns and trends in user behavior
"""
...
class LearnedPattern:
    ...
    pattern_type: str  # e.g., "sentiment", "topic", "response_preference"
    value: str
    frequency: int = 0
    last_seen: datetime = field(default_factory=datetime.now)
    associated_emotions: Dict[EmotionType, float] = field(default_factory=dict)
    effectiveness_score: float = 0.5  # 0-1, how well this pattern leads to positive outcomes
    
    def decay_frequency(self, steps: int = 1) -> None:
        """Reduce frequency over time (forgetting)."""
        self.frequency = max(0, self.frequency - steps)


class UserProfileAnalyzer:
    """Analyzes user inputs to extract patterns and preferences."""
    ...
    def analyze_sentiment(self, text: str) -> Tuple[str, float]:
        ...
        Returns:
            (sentiment: positive/negative/neutral, intensity: 0-1)
        """
    ...
    def _estimate_formality(self, text: str) -> str:
        """Estimate how formal/casual the user is being."""
...
class AdaptiveLearningEngine:
    """
    Learns from interactions and adapts AI behavior.
    
    Features:
    - Tracks user preferences and patterns
    - Adjusts AI personality based on learning
    - Adapts response strategies
    ...
    def __init__(self, agent_id: str):
        ...
        self.topic_preferences: Dict[str, float] = defaultdict(float)  # 0-1 preference
    ...
    def learn_from_interaction(
        ...
        user_input: str,
        ai_response: str,
        outcome_quality: float = 0.5  # 0-1, how good was the outcome
    ) -> None:
        """
    ...
    def _learn_topic_preferences(self, topics: List[str], quality: float) -> None:
        """Learn what topics lead to good outcomes."""
        for topic in topics:
    ...
    def _update_user_profile(self) -> None:
        """Update overall user profile based on learned patterns."""
    ...
    def get_personality_adjustments(self) -> Dict[PersonalityTrait, float]:
        """
        Get personality trait adjustments based on learning.
        
        Returns:
            Dict mapping traits to adjustment deltas (-0.5 to 0.5)
        """
        adjustments = {}
        
        # Adjust based on user communication style
        if self.user_profile.get("dominant_style") == "emotional":
            adjustments[PersonalityTrait.WARMTH] = 0.15  # Be warmer
        ...
        # Adjust based on user sentiment
        ...
        # Adjust based on topic preferences
    ...
    def get_recommended_strategies(self, n: int = 3) -> List[ResponseStrategy]:
        """
        Get recommended response strategies based on learning.
        
        Returns:
            List of ResponseStrategy enums ranked by effectiveness
        """
        # For now, recommend based on user profile
        if self.user_profile.get("dominant_style") == "emotional":
            return [
    ...
    def decay_learning(self, steps: int = 1) -> None:
        """Reduce weight of old patterns (forgetting over time)."""
        for patterns_list in self.learned_patterns.values():
            for pattern in patterns_list:
                pattern.decay_frequency(steps)
        
        # Remove patterns with zero frequency
